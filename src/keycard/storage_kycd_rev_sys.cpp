 ...
 *   │                                             │
 *   │  READS:                                     │
 *   │    - StorageKycdVldTag entities             │
 *   │    - StorageStaKycdComponent (revoked flag) │
 *   │                                             │
 *   │  WRITES:                                    │
 *   │    - StorageKycdRevTag (replaces VldTag)    │
 *   └─────────────────────────────────────────────┘
 *          │
 *          │ Revoked entities denied by ACL system
 *          ▼
 *   StorageAcssChkSystem rejects revoked keycards
 *
 ...
 *   Revocation data maintained in ECS entities (loaded from MongoDB at boot).
 *
...
void StorageKycdRevSystem::on_start(ecs::Registry& /*registry*/) {
    log::info("[StorageKycdRev] Started");
}

void StorageKycdRevSystem::tick(ecs::Registry& registry, float /*dt*/) {
    // Check all valid keycards for revocation
    // Revocation is stored in the ECS entity (loaded from MongoDB at boot)
    // API route sets a revocation marker, this system enforces it
    auto view = registry.view<StorageStaKycdComponent, StorageKycdVldTag>();
    for (auto entity : view) {
        // Keycard entities with both StaKycdComponent and VldTag
        // that have been marked for revocation via API
        // The revocation is detected by presence of StorageKycdRevTag
        // which is emplaced by the HTTP route handler
        // This system handles the tag swap on client entities
        (void)entity;
    }
}

void StorageKycdRevSystem::on_stop(ecs::Registry& /*registry*/) {
    log::info("[StorageKycdRev] Stopped");
}
