 ...
 * =============================================================================
 *
 * @file        storage_tag_wflw_retr.hpp
 * @brief       StorageWflwRetrTag - retired build inside the retention window
 * @description Marker for StorageWflwRetrComponent entities whose 90-day
 *              retention has not elapsed yet (cleanup filter)
 *
 * @module      ase-storage
 * @layer       3 (Module)
 * @category    tag
 ...
 * @brief StorageWflwRetrTag - retired build inside the retention window
 *
 * State: Asset label = retired; files still on disk, retention running
 * Filter: Used by StorageWflwClnSystem (Tag-filtered retention sweep)
 * Added: StorageWflwTranSystem when the applied target label is "retired"
 * Removed: Entity destroyed by StorageWflwClnSystem after file deletion
 */
struct StorageWflwRetrTag {};
