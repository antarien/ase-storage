 ...
 *
 * @file        storage_relm_arcv_tag.hpp
 * @brief       StorageRelmArcvTag - Der Bereich ist nur noch lesbar, vor der Loeschung
 * @description Marke auf einer Bereichs-Entity: Schreibzugriffe werden abgewiesen, der
 *              Bestand bleibt lesbar, bis er entfernt wird.
 *
 *              ZWEI DINGE AM ALTEN NAMEN WAREN FALSCH. Die Datei hiess
 *              `storage_tag_relm_archived.hpp`: das Typwort `tag` stand in der MITTE, wo
 *              nach INST_ASE_ECS_SER Taxonomie steht. Und `archived` war ausgeschrieben;
 *              der Katalog fuehrt dafuer `arcv` (02_sta_state.json, sta/trs/perm).
 *
 * storage = module, relm = realm, arcv = archived
 *
 * @module      ase-storage
 * @layer       3 (Module)
 * @category    tag/realm
...
/**
 * @brief StorageRelmArcvTag - Realm is read-only (scheduled for deletion)
 *
 * State: Writes rejected, reads still served
 * Filter: registry.view<StorageStaRelmComponent, StorageRelmArcvTag>()
 * Added: Admin archives realm
 * Removed: Realm entity destroyed
 */
struct StorageRelmArcvTag {};
