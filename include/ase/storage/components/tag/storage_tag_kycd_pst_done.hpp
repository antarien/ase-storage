 ...
 * =============================================================================
 *
 * @file        storage_tag_kycd_pst_done.hpp
 * @brief       StorageKycdPstDoneTag - Keycard durable record already emitted
 * @description Idempotency marker on a keycard entity whose durable record has
 *              been emitted, so re-iteration never double-writes the record.
 *
 * @module      ase-storage
 * @layer       3 (Module)
 * @category    tag
 ...
 * @brief StorageKycdPstDoneTag - Keycard durable record already emitted
 *
 * State: Durable keycard record emitted to the Replica for this keycard
 * Filter: StorageKycdPstEmitSystem excludes already-emitted keycards
 * Added: StorageKycdPstEmitSystem after emitting the durable record
 * Removed: never (terminal idempotency marker for the keycard's lifetime)
 */
struct StorageKycdPstDoneTag {};
