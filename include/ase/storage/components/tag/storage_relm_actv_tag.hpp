 ...
 *
 * @file        storage_relm_actv_tag.hpp
 * @brief       StorageRelmActvTag - Der Bereich ist im Normalbetrieb
 * @description Marke auf einer Bereichs-Entity: Lesen und Schreiben sind frei, die
 *              Zugriffspruefung folgt den gewoehnlichen ACL-Regeln.
 *
 *              ZWEI DINGE AM ALTEN NAMEN WAREN FALSCH. Die Datei hiess
 *              `storage_tag_relm_active.hpp`: das Typwort `tag` stand in der MITTE, wo nach
 *              INST_ASE_ECS_SER Taxonomie steht. Und `active` war ausgeschrieben; der
 *              Katalog fuehrt dafuer `actv` (02_sta_state.json, sta/lfc/actv). Der Struct
 *              zieht mit, weil das Namenstor Datei und Struct gegeneinander prueft.
 *
 * storage = module, relm = realm, actv = active
 *
 * @module      ase-storage
 * @layer       3 (Module)
 * @category    tag/realm
...
/**
 * @brief StorageRelmActvTag - Realm has full read-write operation
 *
 * State: Normal operation, all access checks use standard ACL rules
 * Filter: registry.view<StorageStaRelmComponent, StorageRelmActvTag>()
 * Added: StgIniSystem on realm creation (default status)
 * Removed: Admin suspends realm (replaced by StorageRelmSuspTag)
 */
struct StorageRelmActvTag {};
