 ...
 * =============================================================================
 *
 * @file        storage_tag_relm_archived.hpp
 * @brief       StorageRelmArchivedTag - Realm is read-only before deletion
 * @description Status tag for realms marked for deletion with read-only access
 *
 * @module      ase-storage
 * @layer       3 (Module)
 * @category    tag
...
/**
 * @brief StorageRelmArchivedTag - Realm is read-only (scheduled for deletion)
 *
 * State: Read access allowed, write/delete denied, deletion scheduled
 * Filter: StgAclChkSystem allows READ but denies WRITE/DELETE
 * Added: Owner or admin archives realm via API
 * Removed: Realm entity destroyed during cleanup
 */
struct StorageRelmArchivedTag {};
